...
client.on("message", function(message) {
  ...
  if (args[0].toLowerCase() === prefix + "kick") {
    ...
    if (
      ...
      return message.channel.send(
        "**Kingtool :** *I Cant kick this member !* "
      );
  ...
  if (args[0].toLowerCase() == `${prefix}help`) {
    const emb = new Discord.RichEmbed()
      ...
      .setDescription(
        `
        **.🛡️ Moderation:**
        **✦-** \`+ban\`: *Ban a Member* ; **✦-**  \`+kick\`: *Kick a Member.* 
        **✦-** \`+mute\`: *Mute a Member* ; **✦-** \`+unmute\`: *Unmute a Member.*
        **✦-** \`+lock\`: *Lock a T.Channel* ; **✦-**  \`+unlock\`: *Unlock a T.Channel.*
        **✦-** \`+vlock\`: *Lock a V.Channel* ; **✦-** \`+vunlock\`: *Unlock a V.Channel.*
        **✦-** \`+move\`: *Move a Member* ; **✦-** \`+invinfo\`: *Get Info of an Invite.*
        **✦-** \`+clear\`: *Clear Messages.* ; **✦-** \`+unban\`: *Unban a Member.* 

        **⋅😂 fun: 🐸 Memes commande:**
        **✦-** \`+love\`: *taq sameone* ; **✦-**  \`+kiss\`: *tag someone.*
        **✦-** \`+hug\`: *tag someone.**✦-**  \`+slap\`: *tag someone.*
        **✦-** \`+memes\`: *to get memes*

        **⋅🎲 games commande:**
        **✦-** \`+slots\`: *Play the Slots* 
        **✦-** \`+xo\`: *tag someone* /**✦-**  \`+kill\`: *tag someone.*

        **⋅🛠 Utilities:** **⋅🎫Economy:**
        **✦-** \`+avatar\`: *Check your Avatar* ; **✦-**  \`+server\`: *Check server Info.*
        **✦-** \`+user\`: *Check your Info* ; **✦-** \`+report\`: *Report a Memeber.*
        **✦-**  \`+ping\`: *Your NET Speed* ; **✦-** \`+allbots\`: *Check Server bots.*
        **✦-** \`+roll\`: *Random Number.*; **✦-** \`+tax\`: *Random Number.*
        **✦-** \`+credit\` ; **✦-**  \`+daily\`

        **⋅🎶 Music:**
        **✦-** \`+Play\`: *Play Song* ; **✦-** \`+Pause\`: *pause song*
        **✦-** \`+Resume\`: *Resume Song* ; **✦-** \`+stop \`: *stop song*
        **✦-** \`+forceskip\`: *Skiped song* ; **✦-** \`+Queue\`: *list song*
        **✦-** \`+skipto\`: *Skipto* ; **✦-** \`+Skip\`: *skip song*
        **✦-** \`+Volume\`: *Volume +* ; **✦-** \`+Nowplaying\`: *Now palying*
        **✦-** \`+Leave\`: *Leave channel*  ; **✦-** \`+repeat\`: *Loop song* 

[𝗦𝘂𝗽𝗽𝗼𝗿𝘁](https://discord.gg/2vE6kJ) | [Invite Me](https://discord.com/oauth2/authorize?client_id=758243946488397824&permissions=8&scope=bot)`)
...
client.on("message", msg => {
  ...
  if (msg.content.startsWith(prefix + "report")) {
    ...
    var channel = msg.guild.channels.find(chann => chann.name === "kd-reports");
...
client.login("NzU4MjQzOTQ2NDg4Mzk3ODI0.X2sHtA.7QqTEECPdIkOIhW4tNw07rsxDvQ");
